...
function sousbooking($connexion, $type_intervention, $IDp)
{
    ...
    if(empty($creneau_sous_booking[0]) and strtotime($heure_now) < strtotime("-".$duree_intervention." minute", strtotime("20:00:00"))) // si le créneau de sousbooking est libre et que l'heure est inférieure à 20h - la durée d'une intervention
    {
        ...
        $request_creneaux = "SELECT IDc, Date_creneau, Heure_debut, Heure_fin, Date_priseRDV, IDp, Niveau_priorite, Deplacement FROM Creneaux WHERE Nom_intervention = '$type_intervention' AND Date_creneau = DATE('$date') ORDER BY Date_creneau ASC, Heure_debut ASC"; // requête de récupération des créneaux du jour
        ...
        if(empty($creneaux_du_jour)) // si le tableau est vide == pas de créneaux dans la journée
        {
            ...
            {
                print("L'urgence a bien été gérée. Merci de prendre en note les modifications de l'emploi du temps");
        ...
        else // si le tableau de créneau n'est pas vide
        {
            ...
            while ($i < $size) // tant que l'on a pas parcouru tous les créneaux
            {
                ...
                if ( $creneau_flottant['Niveau_priorite'] > $creneaux_du_jour[$i]['Niveau_priorite'] and $heure_now < $creneaux_du_jour[$i]['Heure_debut']) // si le créneau flottant a un niveau de priorité > au créneau du jour considéré
                {
                    ...
                    print("<br>");
                    print("Veuillez consulter votre emploi du temps");
                    print("<br>");
...
function surbooking($connexion, $type_intervention, $IDp, $duree_intervention, $nb_jours = 0, $creneau_flottant = NULL, $heure_now = NULL)
{
    ...
    $request_creneaux = "SELECT IDc, Date_creneau, Heure_debut, Heure_fin, Date_priseRDV, IDp, Niveau_priorite, Deplacement FROM Creneaux WHERE Nom_intervention = '$type_intervention' AND Date_creneau = DATE('$date_considérée') ORDER BY Date_creneau ASC, Heure_debut ASC"; // requête de récupération des créneaux du jour
    ...
    else
    {
        ...
        while ($i < $size) // on parcourt les créneaux de la journée
        {
            ...
            {
                print("Le créneau que l'on va décaler est : ".$creneaux_du_jour[$i]['Heure_debut']."-".$creneaux_du_jour[$i]['Heure_fin']." le ".$creneaux_du_jour[$i]['Date_creneau'].".");
                print("<br>");
                print("Veuillez consulter votre emploi du temps");
                print("<br>");
